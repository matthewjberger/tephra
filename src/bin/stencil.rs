...
impl Command for DemoApp {
    ...
    fn recreate_pipelines(
        ...
        self.pipeline = None;
        self.pipeline = Some(RenderPipeline::new(context.clone(), settings));

        let shader_paths = ShaderPathSetBuilder::default()
            .vertex("assets/shaders/model/model_outline.vert.spv")
            .fragment("assets/shaders/model/model_outline.frag.spv")
            .build()?;
        let shader_set = shader_cache.create_shader_set(context.clone(), &shader_paths)?;
